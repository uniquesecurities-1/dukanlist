  ...
  function card(b){
    ...
    return '<article style="position:relative;background:#fff;border:2px solid #FED7AA;border-radius:14px;padding:0;display:flex;flex-direction:column;gap:0;box-shadow:0 2px 6px rgba(15,23,42,.05);cursor:pointer;overflow:hidden" onclick="if(!event.target.closest(\'a,button\')){window.location.href=\'/business.html?slug=' + slug + '\';}">'
      + (thumb
          ...
          : '<div style="width:100%;position:relative;padding-bottom:62.5%;overflow:hidden;background:linear-gradient(135deg,#FFF7ED 0%,#FED7AA 40%,#FFB870 100%)"><div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:16px;background-image:radial-gradient(circle at 20% 20%, rgba(255,255,255,.5), transparent 45%),radial-gradient(circle at 80% 80%, rgba(255,107,26,.15), transparent 55%)"><div style="font-size:2.6rem;line-height:1;margin-bottom:6px;filter:drop-shadow(0 2px 4px rgba(120,53,15,.15))">' + esc(catIcon) + '</div><div style="font-family:\'Manrope\',sans-serif;font-size:.95rem;font-weight:900;color:#7C2D12;letter-spacing:-.01em;line-height:1.15;text-shadow:0 1px 2px rgba(255,255,255,.5);max-width:90%">' + esc(b.name || '') + '</div><div style="font-size:.68rem;font-weight:800;color:#9A3412;margin-top:6px;letter-spacing:.08em;text-transform:uppercase;opacity:.85">' + esc(catName) + '</div></div></div>')
