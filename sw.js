/* ============================================================
   ...
   ============================================================ */
const VERSION    = 'dukan-v2.9.33';  // ADMIN FEATURE — admin/shop.html now has a dedicated 📍 Location card to change a shop's city and locality assignment. Use case: Goyal Saree Emporium registered as Sirsa but actually located in Mandi Dabwali — admin had no UI to fix this. New card has: (a) City dropdown loaded from geo_cities table (sorted by name, current pre-selected), (b) Locality dropdown loaded on-demand when city changes (fetches localities table filtered by city_id), (c) shows current values as small text under each select, (d) confirmation modal before save explaining impact on search ranking + URL slug, (e) status indicator, (f) saves via existing admin_update_shop RPC with patch {city_id, locality_id} so the change is audit-logged automatically. Warning banner explains side effects.
