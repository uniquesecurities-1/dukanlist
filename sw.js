/* ============================================================
   ...
   ============================================================ */
const VERSION    = 'dukan-v2.9.51';  // SHARE MESSAGE UPGRADE — user feedback: WhatsApp share preview missing owner name (key trust signal for friend-to-friend referrals). Also broken Unicode � character before category (the generic 🏷️ label emoji wasn't rendering on user's WhatsApp). Fixed: (a) added 👤 Owner/Founder/Partner line using BIZ.owner_name with auto-friendly role label (uses BIZ.owner_role if set, else falls back to 'Owner'). (b) replaced generic 🏷️ emoji with the ACTUAL category icon from DB (BIZ.categories.icon) — uses what owner already configured for their category (⚡ for solar, 💊 for pharmacy, etc.). Unicode-safe — same emoji that renders fine on category cards renders fine here. (c) Added 📍 prefix to location line for visual hierarchy. (d) Changed 'Verified' to 'Verified on DukanList' for stronger brand cue. New format: Business name → Owner role+name → category+icon → location → trust signals → URL → tagline. Personal trust + clear visual hierarchy.
