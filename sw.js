/* ============================================================
   ...
   ============================================================ */
const VERSION    = 'dukan-v2.9.34';  // FIX — admin location editor silently failed because admin_update_shop RPC has a whitelist that did NOT include city_id/locality_id. Patch was being silently dropped, RPC returned success but DB unchanged. Now: (a) created db/129-admin-update-shop-add-location.sql migration that adds city_id + locality_id to v_allowed array (user must run in Supabase SQL Editor for permanent RPC support with audit logging), (b) frontend updated with dual-path save: tries RPC first, VERIFIES the update by re-fetching, if values didn't change falls back to direct UPDATE on businesses table (works without SQL migration). (c) Locality loader now tries both 'localities' and 'geo_localities' table names — handles schema variance. Both city change AND locality change now persist.
