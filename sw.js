/* ============================================================
   ...
   ============================================================ */
const VERSION    = 'dukan-v2.9.41';  // CROSS-BUSINESS REFERRAL NETWORK — Doctor-Optical Effect. Owners can recommend up to 8 trusted partner businesses creating a referral graph that transfers trust. (1) NEW db/130-cross-business-recommendations.sql: business_recommendations table with relationship_label, RLS policies (public read for active, owner write for own, admin all). 5 RPCs: get_my_recommendations() / get_recommendations_for_business(slug) / get_recommenders_of_business(slug) / add_recommendation(id, label) / remove_recommendation(id) / search_businesses_for_recommendation(query). Constraints: no self-recommendation, no duplicates, max 8 per business. (2) NEW panel/recommendations.html: 'Trusted Partners' UI with counter (0/8), card list with category icon + name + city + relationship label, add modal with search input + quick-label suggestions ('Trusted partner' / 'Our preferred supplier' / 'We refer customers here' / 'Local neighbor we trust'), remove confirmation, EN+HI bilingual. (3) business.html: new '🤝 Trusted Partners' section displays businesses this owner recommends — each shows photo/icon, name (clickable), category + city, rating, optional relationship label, with hover effect. Loaded via loadRecommendations() RPC call. (4) Dashboard quick-action tile added with red gradient + 🤝 icon linking to recommendations panel. User must run db/130 SQL in Supabase for backend to work.
