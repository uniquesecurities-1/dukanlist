/* ============================================================
   ...
   ============================================================ */
const VERSION    = 'dukan-v2.9.36';  // FIX QR Poster — 'Could not load business info' error. Root cause: Supabase PostgREST foreign-key resolution syntax 'categories(name, icon), geo_cities(name)' was failing because relationship metadata isn't accessible or the foreign-key constraints aren't named exactly as PostgREST expects. Replaced with: (a) simple fields-only query getting just category_id, sub_category_id, city_id, (b) separate try/catch query to categories table for name+icon, (c) separate try/catch query to geo_cities for name. Both auxiliary queries are non-fatal — even if they fail, poster still renders with shop name + verified status. Also surfaces actual error message in loading state for easier debugging.
