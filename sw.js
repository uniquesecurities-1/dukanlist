/* ============================================================
   ...
   ============================================================ */
const VERSION    = 'dukan-v2.9.45';  // HOTFIX — homepage photos returning 404. Root cause: DukanImg.opt() was converting Supabase Storage URLs from /storage/v1/object/public/ to /storage/v1/render/image/public/ assuming Image Transformation API is enabled. But this requires Supabase Pro plan or specific addon — on the current project it returns 404, breaking ALL homepage photo cards (showed as yellow gradient placeholders). Also font preload URLs (Manrope woff2 hash URLs) returned 404 because I guessed at the hashes. Fix: (1) image-opt.js now returns ORIGINAL URL by default. Added DukanImg.enableTransform flag for future activation when Supabase Pro is purchased. Helper preserved so calling code doesn't need to change. (2) Removed broken font preload links from index.html — kept logo preload + DNS prefetch + preconnect which are always safe. Photos restored.
