-- =========================================================================
-- Migration 203: Enable Owner Updates for Website + Social Media URLs
-- Created: 2026-07-30
--
-- BUG:
--   update_my_business() RPC (from db/193) only handles a fixed set of
--   editable fields. website_url, facebook_url, instagram_url, youtube_url,
--   x_twitter_url, linkedin_url, owner_role, video_url are NOT in the list.
--
--   When profile.html sends { website_url: "https://..." } in the patch,
--   the RPC silently ignores it — no error, no save. User thinks it saved,
--   but DB never changes. Public page never shows the URL.
--
-- FIX:
--   Replace update_my_business() with an extended version that handles
--   all these owner-editable fields. Same URL validation as WhatsApp.
--
-- SAFE TO RE-RUN.
-- =========================================================================
...
BEGIN
  ...
  -- Helper: normalize URL (add https:// prefix if missing, trim)
  -- Applied inline to each field.
...
--
-- VERIFY:
--   -- Test that website_url now saves. Log into owner panel, edit
--   -- Website URL in Social Media card, save, then check:
--   SELECT website_url, facebook_url, instagram_url, youtube_url
--     FROM businesses WHERE slug = 'your-shop-slug';
-- =========================================================================
