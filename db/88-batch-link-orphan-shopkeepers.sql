...
-- values (non-UUID strings) do not crash the query.
-- =====================================================
...
BEGIN
  ...
  IF v_deleted_biz > 0 THEN
    RAISE NOTICE 'WARN: % users have orphan auth accounts because their', v_deleted_biz;
  ...
  END IF;
  IF v_orphans = 0 THEN
    RAISE NOTICE 'SUCCESS: ALL USERS LINKED OR HANDLED. Photo upload ready.';
  END IF;
  RAISE NOTICE '====================================================';
END $$;
