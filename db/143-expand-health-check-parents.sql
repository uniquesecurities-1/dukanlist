-- ============================================================
-- db/143 — Expand admin_health_check critical-categories list
-- ============================================================
-- Audit finding: db/139's admin_health_check verifies only 8 of the
-- 15 actual parent categories in the system. The 7 unchecked parents
-- (financial-services, insurance, construction-material, beauty-wellness,
-- agri-business, energy-solar, community-social) could silently vanish
-- and the admin health dashboard would still report "all green".
--
-- This migration re-creates the EXACT same admin_health_check function
-- from db/139, with the v_critical_categories array expanded to all 15
-- parents. Every other check (RPCs, data quality, abuse signals, etc.)
-- is preserved byte-for-byte.
--
-- IDEMPOTENT: CREATE OR REPLACE (same as db/139).
-- DB never disturbed — function replacement only, no data touched.
-- ============================================================
...
DECLARE
  ...
  v_critical_categories TEXT[] := ARRAY[
    ...
    -- Added by db/143 — the 7 parents that were silently unchecked
    'financial-services','insurance','construction-material','beauty-wellness',
    'agri-business','energy-solar','community-social'
  ];
