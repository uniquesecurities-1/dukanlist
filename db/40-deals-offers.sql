-- =====================================================
-- db/40-deals-offers.sql
-- Deals & Offers — Light Implementation
-- =====================================================
-- DESIGN:
--   • Text-mostly deals (title + body + discount % or text + dates)
--   • Optional image URL (no required upload — keeps payload tiny)
--   • Max 5 active deals per shop (enforced by trigger)
--   • Auto-expire via valid_until — no cron needed
...
CREATE INDEX IF NOT EXISTS idx_deals_active   ON deals(business_id, valid_until DESC) WHERE valid_until > NOW();
CREATE INDEX IF NOT EXISTS idx_deals_global   ON deals(valid_until DESC) WHERE valid_until > NOW();
CREATE INDEX IF NOT EXISTS idx_deals_business ON deals(business_id, created_at DESC);

ALTER TABLE deals ENABLE ROW LEVEL SECURITY;

-- RLS — public can read active, owner can manage own
...
-- ---------- 2. Max 5 active deals enforcement ----------
...
BEGIN
  IF NEW.valid_until <= NOW() THEN
    RETURN NEW;  -- inserting an already-expired deal (admin maybe). Don't count.
  END IF;
...
-- ---------- 4. my_deals — owner's deals (active + recently expired) ----------
...
LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE AS $$
  SELECT d.id, d.business_id, d.title, d.body, d.discount_pct, d.discount_text,
   ...
   WHERE d.business_id IN (SELECT bo.business_id FROM business_owners bo WHERE bo.auth_user_id = auth.uid())
     AND d.valid_until > NOW() - INTERVAL '30 days'  -- include recently expired so owner can see history
...
BEGIN
  ...
  -- Content keyword check on title + body
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- check_content_violations may not exist; skip silently
  END;
...
-- ---------- 9. business_deals_count — for card badges ----------
