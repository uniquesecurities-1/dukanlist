-- ============================================================
-- db/172 — admin_soft_add_shop: auto-derive pincode from city
-- ============================================================
-- BUG (user screenshot):
--   null value in column "pincode" of relation "businesses"
--   violates not-null constraint
--
-- ROOT CAUSE:
--   businesses.pincode is NOT NULL (db/01-schema line 107). Just
--   like db/171 fixed district_id, we forgot pincode. geo_cities
--   has a pincodes TEXT[] column with the city's PIN codes.
--
-- FIX:
--   When deriving district from city (db/171's join), also fetch
--   the first pincode from c.pincodes[1]. Fallback to '000000' if
--   the city has no pincodes seeded — safer than blowing up the
--   insert, and admin can fix later from manage page.
--
-- Applies to: admin_soft_add_shop (single) + admin_soft_bulk_add
-- (paste). Both get the same fix.
-- ============================================================

BEGIN;

-- ============================================================
-- PART 1: admin_soft_add_shop (single add)
-- ============================================================
...
BEGIN
  ...
  -- Derive district, state, AND pincode in one shot
  SELECT
    c.district_id,
    d.state_id,
    COALESCE(c.pincodes[1], '000000')   -- first pincode, fallback if empty
...
-- ============================================================
-- PART 2: admin_soft_bulk_add (paste names)
-- ============================================================
...
DO $$ BEGIN
  RAISE NOTICE 'db/172 installed. admin_soft_add_shop + admin_soft_bulk_add now derive pincode from city.';
END $$;
