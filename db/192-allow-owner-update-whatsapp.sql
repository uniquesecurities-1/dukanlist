-- =========================================================================
-- Migration 192: Allow owner to update WhatsApp number from profile edit
-- Created: 2026-07-15
-- Author:  Deepak Singla / DigiMutual Goals Pvt. Ltd.
--
-- BACKGROUND:
--   db/91-lock-identity-fields-and-instant-edits.sql previously locked
--   whatsapp field from owner edits (identity/anti-abuse policy).
--   However, Golden Pages pivot needs owner to be able to update their
--   WhatsApp number themselves without contacting admin.
--
--   This migration reintroduces `whatsapp` as an owner-editable field
--   in the update_my_business RPC. All other locked fields remain
--   admin-only:
...
--   Wraps existing update_my_business() logic and adds a whatsapp
--   handler at the top so owners can patch their WhatsApp number.
--
-- SAFE TO RE-RUN.
-- =========================================================================
...
BEGIN
  ...
  -- ===== OWNER-EDITABLE FIELDS (all instant) =====
  ...
  IF p_patch ? 'whatsapp' THEN
    ...
    IF COALESCE(v_new_val,'') IS DISTINCT FROM COALESCE(v_old_val,'') THEN
      -- Basic sanity: strip non-digits, require 10 or 12 digits
  ...
  -- ===== LOCKED FIELDS (silently ignored — admin only) =====
  --   name, address_line1, address_line2, pincode,
  --   city_id, locality_id, state_id, district_id,
  --   mobile, email
  --
  -- If a patch tries to set these, do nothing. No error, no log.
  -- Frontend inputs are already locked for defense-in-depth.
...
-- 5. Test from owner profile page — WhatsApp update should persist
-- =========================================================================
